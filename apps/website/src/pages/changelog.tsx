    ...
    sections: [
      {
        ...
        changes: [
          ...
          {
            text: 'Add Copilot-specific provider to properly handle reasoning tokens',
            contributor: '@SteffenDE',
          },
          {
            text: 'Respect QWERY_MODELS_URL environment variable in build process',
            contributor: '@dbartels',
          },
          {
            text: 'Use snake_case for thinking parameter with OpenAI-compatible APIs',
            contributor: '@Chesars',
          },
          { text: 'Bump AI SDK packages' },
          { text: "Ensure ask question tool isn't included when using acp" },
          {
            text: 'Handle redirected statement treesitter node in bash permissions',
            contributor: '@pschiel',
          },
          {
            text: 'Remove special case handling for Google Vertex Anthropic provider in response generation',
            contributor: '@MichaelYochpaz',
          },
      ...
      },
      {
        ...
        changes: [
          ...
          {
            text: 'Refactored tests and added project tests',
            contributor: '@neriousy',
          },
        ],
      },
    ],
    ...
    sections: [
      {
        ...
        changes: [
          ...
          {
            text: 'Added support for custom authentication providers',
            contributor: '@jsmith',
          },
...
export default function Changelog() {
  return (
    <div className="bg-background min-h-screen" data-testid="page-changelog">
      <header
          ...
          <div className="flex h-16 items-center justify-between gap-4">
            <Link href="/" data-testid="link-changelog-home">
              <span
                ...
                data-testid="wrap-changelog-brand"
              >
                <img
                  src={logoImg}
                  alt="Qwery"
                ...
                <span
                  ...
                  data-testid="text-changelog-brand"
                >
                  qwery
                </span>
              </span>
            ...
            <div
              ...
              data-testid="row-changelog-actions"
            >
              <Link href="/docs" data-testid="link-changelog-docs">
                <Button
                  variant="secondary"
                  className="nb-button bg-card h-9 px-3"
                  data-testid="button-changelog-docs"
                >
                  Docs
          ...
          <div className="py-14 sm:py-20">
            <div className="mb-10" data-testid="wrap-changelog-hero">
              ...
              <p
                ...
                data-testid="text-changelog-description"
              >
            ...
            <div className="space-y-6" data-testid="list-changelog-entries">
              {changelog.map((entry) => (
                ...
                >
                  <div
                    className="border-foreground/20 flex items-center gap-4 border-b pb-4"
                    data-testid={`header-${entry.version}`}
                  >
                    <div
                      className="nb-chip px-3 py-1 font-mono text-sm font-semibold"
                  ...
                  <div
                    ...
                    data-testid={`content-${entry.version}`}
                  >
                    {entry.sections.map((section) => (
                      <div
                        key={section.title}
                        data-testid={`section-${entry.version}-${section.title.toLowerCase()}`}
                      >
                        <h3
                          className="font-semibold"
                          data-testid={`title-${section.title.toLowerCase()}`}
                        >
                          {section.title}
                        </h3>
                        <ul
                          className="mt-3 space-y-2"
                          data-testid={`list-${section.title.toLowerCase()}`}
                        >
                          {section.changes.map((change, idx) => (
                            ...
                            >
                              <span className="bg-muted-foreground mt-2 h-1.5 w-1.5 shrink-0 rounded-full" />
                              <span>
                                {change.text}
                                {change.contributor && (
