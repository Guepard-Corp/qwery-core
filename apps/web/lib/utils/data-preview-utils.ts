...
async function getDuckDB() {
    ...
    const MANUAL_BUNDLES: duckdb.DuckDBBundles = {
        mvp: {
            ...
            mainWorker: '/extensions/duckdb-wasm.default/duckdb-browser-mvp.worker.js',
        },
        eh: {
 ...
 */
async function fetchDataFromServer(url: string, limit: number, format: 'parquet' | 'csv'): Promise<DataFetchResult> {
    try {
        const datasourceProvider = format === 'parquet' ? 'parquet-online' : 'gsheet-csv';
        const driverId = format === 'parquet' ? 'parquet-online.duckdb' : 'gsheet-csv.csv';
 ...
 */
async function fetchData(url: string, format: 'parquet' | 'csv', limit: number = DATA_PREVIEW_CONFIG.INITIAL_LIMIT): Promise<DataFetchResult> {
    ...
    try {
        ...
        try {
            ...
            if (contentLength && parseInt(contentLength, 10) > DATA_PREVIEW_CONFIG.MAX_FILE_SIZE) {
                return {
                    data: null,
        ...
        await duckDB.registerFileURL(fileName, url, duckdb.DuckDBDataProtocol.HTTP, false);
        const conn = await duckDB.connect();

        try {
            const query = format === 'parquet'
            ...
            if (errorMessage.includes('XMLHttpRequest') || errorMessage.includes('NetworkError') || errorMessage.includes('CORS')) {
                return fetchDataFromServer(url, limit, format);
            }
            return { data: null, error: queryError instanceof Error ? queryError.message : `Failed to query ${format} file` };
        } finally {
            await conn.close();
            try { await duckDB.dropFile(fileName); } catch (e) { }
        }
    } catch (error) {
        console.warn(`DuckDB initialization failed for ${format}, trying server fallback...`, error);
        return fetchDataFromServer(url, limit, format);
    }
}

export const fetchParquetData = (url: string, limit: number = DATA_PREVIEW_CONFIG.INITIAL_LIMIT) => fetchData(url, 'parquet', limit);
export const fetchCsvData = (url: string, limit: number = DATA_PREVIEW_CONFIG.INITIAL_LIMIT) => fetchData(url, 'csv', limit);
