...
async function getOrCreateAgent(
  ...
  model: string = 'llamacpp/llama-2-7b-chat',
...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  const model: string = body.model || 'llamacpp/llama-2-7b-chat';
  ...
  try {
    ...
    if (datasources && datasources.length > 0 && conversation) {
      ...
      if (datasourcesChanged) {
        console.log(
          `[Chat API] Updating conversation datasources from [${currentDatasources.join(
            ', ',
          )}] to [${datasources.join(', ')}]`,
        );
        ...
        if (updatedConversation) {
          ...
          console.log(
            `[Chat API] Conversation datasources updated to: [${
              updatedConversation.datasources?.join(', ') || 'none'
            }]`,
          );
        } else {
          console.warn(
...
User request: ${cleanText}`;
    ...
    // Enregistrer la réponse de l'assistant dans la base
    await repositories.message.create({
      id: crypto.randomUUID(),
      conversationId: conversation!.id,
      role: MessageRole.ASSISTANT,
      content: { parts: [{ type: 'text', text: data.content }] },
      metadata: {},
      createdAt: new Date(),
      updatedAt: new Date(),
      createdBy: 'assistant',
