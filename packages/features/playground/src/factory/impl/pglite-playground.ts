...
export class PGlitePlayground implements PlaygroundDatabase {
  ...
  async seed(driver: IDataSourceDriver): Promise<void> {
    for (const table of PLAYGROUND_TABLES) {
      await driver.query(table.createTable);

      const countResult = await driver.query(getTableCountQuery(table.name));
      const count =
        (countResult.rows[0] as { count: number | string })?.count ?? 0;
      const countNum = typeof count === 'string' ? parseInt(count, 10) : count;

      if (countNum === 0) {
        await driver.query(table.insertData);
      }
    }
  }
}
