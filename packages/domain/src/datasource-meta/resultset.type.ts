...
export const DatasourceResultStatSchema = z
  .object({
    rowsAffected: z.number().int().min(0).describe('Number of rows affected by the query'),
    rowsRead: z.number().int().min(0).nullable().describe('Number of rows read during query execution'),
    rowsWritten: z.number().int().min(0).nullable().describe('Number of rows written during query execution'),
    queryDurationMs: z.number().min(0).nullable().describe('Query execution duration in milliseconds'),
  })
  .passthrough();
...
export const ColumnHeaderSchema = z
  .object({
     ...
     */
    name: z.string().min(1).describe('The key of row data that this column represents'),
    /**
     ...
     */
    originalType: z.string().nullable().describe('The original database-specific type of the column'),
    /**
     ...
     */
    type: ColumnTypeSchema.optional().describe('Normalized type hint for frontend visualization'),
    /**
     ...
     */
    originalName: z.string().optional().describe('The original column name returned from database driver'),
    /**
     * Indicate if this column is a primary key.
     */
    primaryKey: z.boolean().optional().describe('Indicate if this column is a primary key'),
    /**
     * The column id in the table. Useful for Postgres and other databases that expose column OIDs.
     */
    columnId: z.number().int().optional().describe('The column id in the table (useful for Postgres OIDs)'),
    /**
     * The table id in the database. Useful for Postgres and other databases that expose table OIDs.
     */
    tableId: z.number().int().optional().describe('The table id in the database (useful for Postgres OIDs)'),
  })
  .passthrough();
