...
describe('DatasourceRepository Secrets', () => {
    ...
    beforeEach(async () => {
        ...
        (getDiscoveredDatasource as any).mockResolvedValue({
            id: 'postgres',
            schema: z.object({
                host: z.string(),
    ...
    it('should encrypt secrets when creating a datasource', async () => {
        ...
        const row = db.prepare('SELECT datasource_config FROM datasources WHERE id = ?').get('ds-1');
    ...
    it('should not re-encrypt already protected secrets', async () => {
        ...
        const created = await repository.create(ds);
        const firstEncrypted = (await (repository as any).db.prepare('SELECT datasource_config FROM datasources WHERE id = ?').get('ds-3')).datasource_config;
        ...
        const secondEncrypted = (await (repository as any).db.prepare('SELECT datasource_config FROM datasources WHERE id = ?').get('ds-3')).datasource_config;

        // The encrypted string should be the same (or at least valid)
