 ...
 */
export function extractGenericUrl(config: Record<string, unknown>, keys: string[]): string | null {
 ...
 */
export function extractPath(config: Record<string, unknown>, keys: string[]): string | null {
 ...
 * Removes channel_binding parameter and normalizes sslmode.
 */
 ...
 */
export function extractConnectionUrl(config: Record<string, unknown>, providerId: string): string {
  const connectionUrl = extractGenericUrl(config, ['connectionUrl', 'url', 'path']);
  if (connectionUrl) {
